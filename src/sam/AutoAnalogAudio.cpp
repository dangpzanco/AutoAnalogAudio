...
AutoAnalog::AutoAnalog():
dataReady(0), dataTimer(0), sampleCount(0),
tcTicks(2625), tcTicks2(2625), adjustDivider(2),
adjustCtr(0), adjustCtr2(0), whichDma(0),
aCtr(0), dacChan(0), whichDac(0),
adcBitsPerSample(8), dacBitsPerSample(8), autoAdjust(true),
adcLastAdjust(0), adcNumSamples(0)
{
    for(int i = 0; i < MAX_BUFFER_SIZE; i++) dacBuffer[i] = 0;
}
    ...
    if(sampleCount < 100){ ++sampleCount; }
        if(sampleCount >= 100 && autoAdjust){
            ...
            if( tt->TC_RC >= t->TC_RC - 25 && adjustCtr2 == 0){
                tcTicks2--;
                tc2Setup();
            } else if( (ADC->ADC_RNCR > 0 || ADC->ADC_RCR > 0 ) ){
                tcTicks2 -= 10;
                tc2Setup();
            } else if( (ADC->ADC_RNCR == 0 && ADC->ADC_RCR == 0 ) && millis() - adcLastAdjust > 500 && tt->TC_RC < t->TC_RC - 35){
...
void AutoAnalog::feedDAC(uint8_t dacChannel, uint32_t samples) {
    ...
    for(int i=0; i<samples; i++){
        ...
        else if(dacBitsPerSample == 10) 
            realBuf[i] = dacBuffer16[i] << 2;
        else 
            realBuf[i] = dacBuffer[i] << 4;

        if(dacChannel == 2) dacChan = !dacChan;
        else dacChan = dacChannel;
